...
name: agentmail
description: >
  API-first email platform designed for AI agents. Create and manage dedicated
  email inboxes, send and receive emails programmatically, and handle
  email-based workflows with webhooks and real-time events. Use when the operator
  needs agent-native programmatic email inboxes — not for the info@
  the organization inbox (use email-triage for that). All sends require the operator's
  explicit per-send approval via Slack.
version: 1.0.0
metadata:
  openclaw:
    requires:
